-- War on Retail — collect customer email at checkout
--
-- Checkout now offers an optional email field (for order receipts). The email
-- rides inside the existing `p_customer` JSONB, so the function signature is
-- unchanged — we only read `p_customer->>'email'` and persist it.
--
-- On conflict we `coalesce(excluded.email, customers.email)`: a returning
-- customer who leaves email blank must never wipe an address an admin already
-- saved. Phone stays the identity key; email is best-effort contact info.
