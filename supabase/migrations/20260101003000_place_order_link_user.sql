-- War on Retail — link orders placed while signed in to the customer's account.
--
-- Same `place_order` as 20260101002700_checkout_email.sql, with ONE change: the
-- customer upsert now stamps `auth.uid()` onto `user_id`. For anon checkout
-- `auth.uid()` is null, so guest orders behave exactly as before. For a
-- signed-in buyer this links their (possibly pre-existing, phone-keyed) customer
-- row to their account so it shows up in their dashboard — `coalesce` on
-- conflict means we never steal a row already owned by someone else.
